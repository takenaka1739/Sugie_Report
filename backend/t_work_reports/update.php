 ...
 *
 * 更新可能フィールド（任意で部分更新）:
 *   start_time, finish_time, is_canceled, alcohol_checked, condition_checked,
...
function norm_work_string_or_null($v) {
    // 「フィールドを送ってない」場合は update 対象外にしたいので、この関数は
    // 呼び出し側で array_key_exists チェック後にのみ使われます。
...
try {
    ...
    $fields = [
        ...
        'payment1_id' => ['type' => 'int'],
        'amount1'     => ['type' => 'int'],
        'payment2_id' => ['type' => 'int'],
        'amount2'     => ['type' => 'int'],
        'payment3_id' => ['type' => 'int'],
        'amount3'     => ['type' => 'int'],
        'payment4_id' => ['type' => 'int'],
        'amount4'     => ['type' => 'int'],
        'payment5_id' => ['type' => 'int'],
        'amount5'     => ['type' => 'int'],
    ];

    if ($hasOnSite2) {
        $fields['on_site_id2'] = ['type' => 'int'];
    }
    if ($hasNight) {
        $fields['is_night_shift'] = ['type' => 'bool01'];
    }
    ...
    foreach ($fields as $key => $meta) {
        ...
        switch ($meta['type']) {
            case 'time':
                $val = norm_time_or_null($raw);
                break;
            case 'bool01':
                $val = norm_bool01_or_null($raw);
                break;
            case 'int':
                $val = norm_int_or_null($raw);
                break;
            case 'work':
                $val = norm_work_string_or_null($raw);
                break;
            default:
                $val = null;
        }
        ...
        $ph = ':' . $key;
        $setParts[] = "{$key} = {$ph}";
        $params[$ph] = $val; // null もOK（明示的にNULLにできる）
    }
