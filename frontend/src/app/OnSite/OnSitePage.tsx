...
// TypeScript用設定：ツールバーに受け渡す追加プロップ（baseAddr）を宣言
...
const EditToolbar = (props: GridSlotProps['toolbar']) => {
  ...
  const handleExportXls = () => {
    const url = `${baseAddr}/m_on_sites/export_xls.php`;
...
const OnSitePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<GridRowsProp>(`${BASE_ADDR}/m_on_sites/select.php`, {
  ...
  const handleDeleteClick = (id: GridRowId) => () => {
    ...
    if (typeof id === 'number') {
      axios.post(`${BASE_ADDR}/m_on_sites/delete.php`, { id }).catch(() => {
  ...
  const processRowUpdate = async (newRow: GridRowModel) => {
    ...
    if (newRow.name === '') {
      window.alert('現場名が未入力のため登録できません。');
      return updatedRow;
    }

    try {
      if ((newRow as any).isNew || isNaN(newRow.id as any)) {
        await axios.post(`${BASE_ADDR}/m_on_sites/insert.php`, {
          name: newRow.name,
          type_id: newRow.type_id,
        });
      } else {
        await axios.post(`${BASE_ADDR}/m_on_sites/update.php`, {
          id: newRow.id,
          name: newRow.name,
          type_id: newRow.type_id,
        });
  ...
  const columns: GridColDef[] = [
    { field: 'name', headerName: '現場名', width: 400, editable: true },
    {
      field: 'type_id',
      headerName: '区分',
      width: 120,
